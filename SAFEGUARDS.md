...
Blockers set `safe: false` and should prevent the operation from proceeding. Currently blockers are only generated by input validation failures:

- Invalid operation name (fails `OperationSchema` validation)
- Tool-level input validation errors (invalid chain name, invalid path, shell metacharacters)

Future versions may add runtime blockers for conditions such as detected live production databases on the target of a destructive operation.
...
| Automation | `setup_scheduled_audit`, `remove_scheduled_audit` |
