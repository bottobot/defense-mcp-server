 ...
 *          policy_evaluate, report, cron_restrict, cron_restrict_status,
 *          tmp_audit, tmp_harden
 */
      ...
      switch (action) {
        ...
        case "lynis_audit": {
          try {
            ...
            if (params.profile) {
              sanitizeArgs([params.profile]);
              args.push("--profile", params.profile);
            }
            if (params.test_group) {
              sanitizeArgs([params.test_group]);
              args.push("--tests-from-group", params.test_group);
            }
            if (params.pentest) {
              args.push("--pentest");
            }
        ...
        case "oscap_scan": {
          try {
            const profile = params.oscap_profile ?? "xccdf_org.ssgproject.content_profile_standard";

            // Auto-detect SCAP content if not provided
            let contentPath = params.content;
            if (!contentPath) {
              const candidates = [
            ...
            if (params.results_file) {
              sanitizeArgs([params.results_file]);
              args.push("--results", params.results_file);
            }
            if (params.report_file) {
              sanitizeArgs([params.report_file]);
              args.push("--report", params.report_file);
            }
            ...
            const output = {
              ...
              resultsFile: params.results_file ?? "not saved",
              reportFile: params.report_file ?? "not saved",
        ...
        case "policy_evaluate": {
          try {
            if (!params.policy_name && !params.policy_path) {
            ...
            if (params.policy_path) {
              sanitizeArgs([params.policy_path]);
              policyFilePath = params.policy_path;
            } else {
              const config = getConfig();
              policyFilePath = `${config.policyDir}/${params.policy_name}.json`;
            }
        ...
        // ── report ───────────────────────────────────────────────────
        case "report": {
          const { format, include_lynis, include_cis, include_policy, report_policy_name } = params;
          try {
            const report: {
              timestamp: string;
              overallScore: number;
            ...
            // ── Policy section ──
            if (include_policy && report_policy_name) {
              try {
                const config = getConfig();
                const policyPath = `${config.policyDir}/${report_policy_name}.json`;
                ...
                report.sections.push({
                  name: `Policy: ${report_policy_name}`,
                  score: 0,
                  details: { error: `Policy '${report_policy_name}' not found or failed to evaluate` },
                });
              }
            }
        ...
        // ── tmp_harden ───────────────────────────────────────────────
        case "tmp_harden": {
          const { mount_options, dry_run } = params;
          try {
