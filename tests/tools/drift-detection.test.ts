/**
 * Tests for src/tools/drift-detection.ts
 *
...
describe("drift-detection tools", () => {
  ...
  it("should register drift_baseline tool", () => {
    expect(tools.has("drift_baseline")).toBe(true);
  });

  it("should default dryRun to true", async () => {
    const handler = tools.get("drift_baseline")!.handler;
    const result = await handler({ action: "create", name: "test", directories: ["/etc"], dryRun: true });
  ...
  it("should return error when comparing non-existent baseline", async () => {
    ...
    const handler = tools.get("drift_baseline")!.handler;
    const result = await handler({ action: "compare", name: "nonexistent" });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain("not found");
  });

  it("should handle create action in non-dry-run mode", async () => {
    ...
    const handler = tools.get("drift_baseline")!.handler;
    const result = await handler({ action: "create", name: "test", directories: ["/etc"], dryRun: false });
    expect(result.isError).toBeUndefined();
  });
